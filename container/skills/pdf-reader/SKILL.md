...
When a user sends a PDF on Telegram, it is automatically downloaded and saved to the group's `attachments/` directory. The incoming message will look like:

> [Document: report.pdf] (/workspace/group/attachments/document_123.pdf)

The path inside the parentheses is the container-relative path. To read it:

```bash
pdf-reader extract /workspace/group/attachments/document_123.pdf --layout
```
