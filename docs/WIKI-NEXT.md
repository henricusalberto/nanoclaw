...
- **Historical message archive importer** (`src/wiki/ingesters/`). Port of farza's gbrain / llm-wiki iMessage + WhatsApp ingesters to TypeScript. Reads `~/Library/Messages/chat.db` and `~/Library/Group Containers/.../ChatStorage.sqlite` read-only, resolves phone numbers via the macOS AddressBook SQLite, filters to DMs, groups by `(contact, day)`, and writes one immutable raw source page per day under `sources/imessage-*.md` / `sources/whatsapp-*.md`. Idempotent — reruns produce no new writes. CLI: `wiki import imessage [--since --top --min-msg-len --apply]` / `wiki import whatsapp [...]`. Requires Full Disk Access on the running terminal. First real run still pending — dry-run verified on this machine 2026-04-11, FDA will unblock the apply.
- **Unified memory: Claude Code ↔ Janus** (2026-04-11). Two changes that close the memory gap between Claude Code sessions and Janus container sessions. Track A: new project skill at `.claude/skills/wrap/SKILL.md` gives Claude Code a `/wrap` command that writes plain-markdown session logs to `groups/global/memory/YYYY-MM-DD-claude-<topic>.md` in the OpenClaw-compatible schema (no frontmatter, H1 + H2 + bullets). Same destination Janus uses, same schema the daily 23:00 CET synth cron expects. Track B: extended `BridgeSourceConfig` with an `absoluteRootPath` field so the wiki bridge can pull files from outside the repo, and added a `claude-code-auto-memory` source pointing at `~/.claude/projects/-Users-kimbehnke--nanoclaw-nanoclaw/memory/`. First bridge run picked up all 10 typed auto-memory files (`user_*`, `feedback_*`, `project_*`, `reference_*`) as `sources/source.claude-code-auto-memory--*.md` source pages with `sourceType: memory-bridge` and `bridgeAgentIds: [claude-code]`. The wiki entity-scanner will absorb them into structured claims on the next dream-cycle pass. Backwards compat with OpenClaw is preserved by schema parity — if the user ever switches back, the `/wrap` skill's docstring documents a one-time `rm -rf ~/.openclaw/workspace/memory && ln -s` migration that points OpenClaw at the live tree.
